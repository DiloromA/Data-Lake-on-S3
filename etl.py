...
def process_song_data(spark, input_data, output_data):
    """ Processes song data by reading from S3 bucket, 
    creating song_table and artist table with select columns, 
    dropping duplicates and writing them back to output S3 location in parquet file.
    
    Arg:
    spark -- spark session
    input_data -- original source of data to consume, S3 bucket location
    output_data -- S3 bucket location to write the processed data     
    """
...
def process_log_data(spark, input_data, output_data):
    """ Processes log data by reading from S3 bucket, 
    creating song_table and artist table with select columns, 
    dropping duplicates, creating additional derived fields (time_table) 
    and writing them back to output S3 location in parquet file.
    ...
    song_df = spark.read.format('json').load(input_data + 'song_data/*/*/*').drop_duplicates()
